    ...
    <>
      ...
      <div className="w-full flex justify-center py-2" style={{ backgroundColor: colors.background }}>
        <div className="w-full max-w-6xl rounded-lg p-2">
            ...
            <tbody>
              {paginatedProducts.map((product) => (
                ...
                >
                  ...
                  <td className="py-2 px-3">${product.productPrice}</td>
                  <td className="py-2 px-3">{product.salesCount}</td>
                  <td className="py-2 px-3">{product.productStock}</td>
                </tr>
              ))}
                      ...
                      {paginatedProducts.length === 0 &&
                        i === Math.floor(emptyRows / 2) && (
                          <div className="flex flex-col items-center justify-center">
                            ...
                            />
                            <span className="text-sm font-semibold">NO SE ENCONTRARON PRODUCTOS</span>
                          </div>
                        )}
                    </td>
                  </tr>
          ...
          {/* Paginación... (mantén tu código de paginación igual) */}
          <div className="mt-4 flex justify-between items-center text-sm text-gray-500 dark:text-gray-400">
            <span>
              Mostrando {filteredProducts.length === 0 ? 0 : startIndex + 1} a {Math.min(startIndex + itemsPerPage, filteredProducts.length)} de {filteredProducts.length} productos
            </span>
            <div className="flex items-center gap-1">
              ...
              <button
                ...
                className="border rounded-md px-3 py-1 text-sm transition"
                style={{
                  backgroundColor: page === totalPages || totalPages === 0 ? colors.muted : 'transparent',
                  color: page === totalPages || totalPages === 0 ? colors.tertiary : colors.textPrimary,
                  borderColor: colors.textDisabled,
